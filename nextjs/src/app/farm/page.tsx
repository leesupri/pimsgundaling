...
const sections = [
  {
    id: "cows",
    title: "Sapi Holstein",
    sub: "Susu Segar Setiap Hari",
    desc: "Ratusan sapi Holstein kami merumput bebas di padang rumput ketinggian 1.400 mdpl. Setiap pagi, susu segar diperah untuk kebutuhan dapur restoran, produksi keju, dan gelato.",
    img: "/images/farm/cows/herd.jpg",
    badge: "🐄 Free Range",
  },
  {
    id: "cheese",
    title: "Ruang Keju",
    sub: "Artisan Cheese Making",
    desc: "Di ruang keju kami, ahli cheese maker mengubah susu segar menjadi 5 varietas keju artisan. Dari Tomme Andaliman yang beraroma rempah Karo hingga Camembert yang lembut.",
    img: "/images/farm/cheese-room/cheese.jpg",
    badge: "🧀 Artisan",
  },
  {
    id: "gardens",
    title: "Kebun Sayur & Buah",
    sub: "Seasonal Farm Garden",
    desc: "Kebun kami menghasilkan sayuran, buah, dan rempah-rempah segar yang menjadi bahan utama menu musiman restoran. Apa yang tumbuh hari ini, tersaji di meja Anda esok hari.",
    img: "/images/farm/gardens/beds.jpg",
    badge: "🌱 Organic",
  },
];

const fieldTripPackages = [
  {
    title: "Paket Edu Farm",
    price: "Rp 75.000 / orang",
    duration: "2 jam",
    items: ["Tur kandang sapi & proses pemerahan", "Pengenalan pembuatan keju", "Makan siang farm to table", "Aktivitas memberi makan sapi"],
  },
  {
    title: "Paket Full Experience",
    price: "Rp 150.000 / orang",
    duration: "Half day",
    items: ["Semua paket Edu Farm +", "Workshop membuat keju (take-home)", "Tur kebun & panen sendiri", "Sesi gelato making", "Oleh-oleh produk farm"],
  },
];
      ...
      <main>
        ...
        <section ref={heroRef} className="relative h-[65vh] min-h-[500px] overflow-hidden flex items-end">
          ...
          <div className="relative z-20 max-w-7xl mx-auto px-6 lg:px-12 pb-16 w-full">
            <motion.span initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              ...
              <span className="w-8 h-px bg-farm-400" />Agrowisata Berastagi
            </motion.span>
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
              ...
              Farm &<br /><span className="text-farm-300 italic">Field Trip</span>
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
              className="text-white/70 mt-4 text-base lg:text-lg max-w-lg">
              Kunjungi peternakan kami dan rasakan pengalaman farm to table yang sesungguhnya.
            </motion.p>
          </div>
        ...
        <section className="py-24 lg:py-32 bg-earth-200">
          <div className="max-w-7xl mx-auto px-6 lg:px-12 space-y-24">
            {sections.map(({ id, title, sub, desc, img, badge }, i) => (
              <AnimatedSection key={id} direction={i % 2 === 0 ? "left" : "right"}>
                <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center ${i % 2 !== 0 ? "lg:grid-flow-dense" : ""}`}>
                  <div className={i % 2 !== 0 ? "lg:col-start-2" : ""}>
                    <span className="inline-flex items-center gap-2 bg-farm-50 text-farm-600 text-xs font-bold px-3 py-1.5 rounded-full mb-4">{badge}</span>
                  ...
                  <div className={i % 2 !== 0 ? "lg:col-start-1" : ""}>
                    <motion.div whileHover={{ scale: 1.02 }} transition={{ duration: 0.4 }} className="relative rounded-3xl overflow-hidden aspect-[4/3] shadow-xl bg-earth-300">
                      <Image src={img} alt={title} fill className="object-cover" sizes="(max-width: 1024px) 100vw, 50vw" onError={() => {}} />
                    </motion.div>
                  </div>
                </div>
              </AnimatedSection>
        ...
        <section id="fieldtrip" className="py-24 bg-farm-900 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
            <AnimatedSection className="text-center mb-14">
              <span className="text-farm-300 text-xs uppercase tracking-[0.25em] font-bold mb-3 block">Education</span>
              <h2 className="font-display font-black text-white text-4xl lg:text-5xl">Paket Field Trip</h2>
              <p className="text-farm-200 mt-3 text-base max-w-xl mx-auto">Tersedia untuk sekolah TK–SMA, komunitas, dan korporat. Minimum 20 orang.</p>
            </AnimatedSection>
            <StaggerContainer className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-4xl mx-auto">
              {fieldTripPackages.map(({ title, price, duration, items }) => (
                  ...
                  <ul className="space-y-2.5">
                    {items.map((item) => (
                      <li key={item} className="flex items-start gap-2 text-farm-200 text-sm">
                        <span className="text-farm-400 mt-0.5 flex-shrink-0">✓</span>{item}
                      </li>
                    ))}
                  </ul>
                </motion.div>
