...
const promoImgs = [
  "/images/promo/brunch.jpg",
  "/images/promo/family.jpg",
  "/images/promo/cheese-box.jpg",
];
      ...
      <main className="bg-earth-200 pt-28">
        <div className="max-w-7xl mx-auto px-6 lg:px-12 pb-24">
          ...
          <AnimatedSection className="mb-14">
            <span className="text-farm-500 text-xs uppercase tracking-[0.25em] font-bold mb-3 block">{t.promo.label}</span>
            <h1 className="font-display font-black text-earth-900 text-5xl lg:text-7xl">
              {t.promo.title1} <span className="text-farm-600 italic">{t.promo.title2}</span>
            </h1>
          </AnimatedSection>

          {/* Promos */}
          <StaggerContainer className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-20">
            {t.promo.promosData.map(({ title, badge, desc, discount }, idx) => (
              <motion.div key={title} variants={staggerItem} whileHover={{ y: -6 }} className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300">
                <div className="relative aspect-[4/3] bg-earth-200 overflow-hidden">
                  <Image src={promoImgs[idx]} alt={title} fill className="object-cover transition-transform duration-500 group-hover:scale-105" onError={() => {}} />
                ...
                <div className="p-6">
                  ...
                  <a href="https://wa.me/6282162599980" target="_blank" rel="noopener noreferrer"
                    ...
                    {t.promo.claimPromo} <span className="group-hover/link:translate-x-1 transition-transform">→</span>
                  </a>
                </div>
              </motion.div>
            ))}
          ...
          <AnimatedSection className="mb-10" id="shop">
            <span className="text-farm-500 text-xs uppercase tracking-[0.25em] font-bold mb-3 block">{t.promo.shopLabel}</span>
            <h2 className="font-display font-black text-earth-900 text-4xl lg:text-5xl">{t.promo.shopTitle}</h2>
            <p className="text-earth-500 mt-2">{t.promo.shopDesc}</p>
          </AnimatedSection>
